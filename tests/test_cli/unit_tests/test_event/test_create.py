...
def test_creation_with_optional_arguments_successful(monkey_token_check_correct_sales, fake_contract, fake_collaborator_support, capsys):
    contract = fake_contract.id
    start_date = "2024-05-20 14:00"
    end_date = "2024-05-20 22:00"
    location = "98, rue des Chats - 92000 NANTERRE"
...
def test_creation_fails_with_wrong_salesman(monkey_token_check_correct_sales_plankton, fake_contract, capsys):
    contract = fake_contract.id
    start_date = "2024-05-20 14:00"
    end_date = "2024-05-20 22:00"
    location = "98, rue des Chats - 92000 NANTERRE"
    ...
    assert "Vous ne pouvez pas créer d'évènement pour un client qui ne vous est pas affecté." in captured.out.strip()
    assert created_event == None
    
def test_creation_fails_with_unsigned_contract(monkey_token_check_correct_sales, fake_contract_unsigned, capsys):
    ...
    assert "Vous ne pouvez pas créer d'évènement pour un contrat qui n'est pas signé." in captured.out.strip()
    assert created_event == None
...
def test_creation_fails_if_not_authenticated(monkey_token_check_false, fake_contract, capsys):
    contract = fake_contract.id
    start_date = "2024-05-20 14:00"
    end_date = "2024-05-20 22:00"
    location = "98, rue des Chats - 92000 NANTERRE"
